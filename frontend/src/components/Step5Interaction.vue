...
const renderMarkdown = (content) => {
  ...
  // Procesar listas - admitir sublistas
...
const sendToAgent = async (message) => {
  ...
  if (res.success && res.data) {
    // Ruta de datos correcta: res.data.result.results es un diccionario de objetos
    // Formato: {"twitter_0": {...}, "reddit_0": {...}} o una sola plataforma {"reddit_0": {...}}
    const resultData = res.data.result || res.data
    const resultsDict = resultData.results || resultData
    
    // Convertir diccionario de objetos a array, priorizar respuesta de plataforma reddit
    let responseContent = null
    const agentId = selectedAgentIndex.value
    
    if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
      // Priorizar respuesta de plataforma reddit, luego twitter
...
/* Left Panel - Report Style (completamente consistente con Step4Report.vue) */
.left-panel.report-style {
  width: 45%;
  min-width: 450px;
  background: #FFFFFF;
