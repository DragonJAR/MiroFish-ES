...
const showEdgeLabels = ref(true) // Mostrar etiquetas de borde por defecto
const expandedSelfLoops = ref(new Set()) // Elementos de bucle propio expandidos
const showSimulationFinishedHint = ref(false) // Indicador después de que termina la simulación
const wasSimulating = ref(false) // Rastrear si estaba en simulación anteriormente
...
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // De simulación a no simulación, mostrar indicador de fin
    showSimulationFinishedHint.value = true
  }
  wasSimulating.value = newValue
}, { immediate: true })

// Alternar estado expandido/contraído de elementos de bucle propio
...
// Calcular tipos de entidad para leyenda
...
const closeDetailPanel = () => {
  ...
  expandedSelfLoops.value = new Set() // Restablecer estado expandido
}
...
const renderGraph = () => {
  ...
  // Procesar datos de bordes, calcular cantidad e índice de bordes entre pares de nodos
  const edgePairCount = {}
  const selfLoopEdges = {} // Bordes de bucle propio agrupados por nodo
  const tempEdges = edgesData
    .filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))
  
  // Contar bordes entre cada par de nodos, recopilar bordes de bucle propio
  tempEdges.forEach(e => {
    if (e.source_node_uuid === e.target_node_uuid) {
      // Bucle propio - recopilar en array
  ...
  // Registrar qué borde de cada par de nodos se está procesando actualmente
  const edgePairIndex = {}
  const processedSelfLoopNodes = new Set() // Nodos de bucle propio ya procesados
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // Bordes de bucle propio - agregar solo un borde combinado por nodo
      ...
      edges.push({
        ...
        type: 'SELF_LOOP',
        name: `${t('graph.selfRelations')} (${allSelfLoops.length})`,
        curvature: 0,
        isSelfLoop: true,
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // Almacenar detalles de todos los bordes de bucle propio
        }
      })
      return
    }
    ...
    // Determinar si la dirección del borde es consistente con la dirección estandarizada (UUID fuente < UUID objetivo)
    const isReversed = e.source_node_uuid > e.target_node_uuid
    
    // Calcular curvatura: dispersar cuando hay múltiples bordes, recto para borde único
    let curvature = 0
    if (totalCount > 1) {
      // Distribuir curvatura uniformemente, asegurar distinción clara
      // Rango de curvatura aumenta según cantidad de bordes, más bordes = rango más grande
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // Si la dirección del borde es opuesta a la dirección estandarizada, invertir curvatura
      // Esto asegura que todos los bordes se distribuyan en el mismo sistema de referencia, sin superposición por diferencias de dirección
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  // Simulación - ajustar espaciado de nodos dinámicamente según cantidad de bordes
  ...
  // Calcular rutas de curvas
  const getLinkPath = (d) => {
    const sx = d.source.x, sy = d.source.y
    const tx = d.target.x, ty = d.target.y
    
    // Detectar bucle propio
    if (d.isSelfLoop) {
      // Bucle propio: dibujar arco circular desde el nodo y volver
      const loopRadius = 30
      // Partir desde el lado derecho del nodo, dar una vuelta y volver
      const x1 = sx + 8  // Desplazamiento de punto de inicio
      const y1 = sy - 4
      const x2 = sx + 8  // Desplazamiento de punto final
      const y2 = sy + 4
      // Usar arco circular para dibujar bucle propio (sweep-flag=1 horario)
    ...
    // Calcular puntos de control de curva - ajustar dinámicamente según cantidad de bordes y distancia
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // Desplazamiento perpendicular a la línea, calculado según proporción de distancia, asegurar visibilidad clara de curva
    // Más bordes = proporción más grande de desplazamiento respecto a distancia
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // Base 25%, agregar 5% por cada borde adicional
  ...
  // Calcular punto medio de curva (para posicionamiento de etiquetas)
  const getLinkMidpoint = (d) => {
    ...
    // Detectar bucle propio
    if (d.isSelfLoop) {
      // Posición de etiqueta de bucle propio: lado derecho del nodo
      return { x: sx + 70, y: sy }
    }
    ...
    // Punto medio de curva Bézier cuadrática t=0.5
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    const pairTotal = d.pairTotal || 1
    ...
    // Fórmula de curva Bézier cuadrática B(t) = (1-t)²P0 + 2(1-t)tP1 + t²P2, t=0.5
  ...
  // Fondo de etiquetas de link (fondo blanco hace el texto más claro)
  ...
  // Guardar referencia para control externo de visibilidad
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Solo registrar posición, no reiniciar simulación (distinguir clic y arrastre)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // Detectar si realmente comenzó el arrastre (movimiento más allá del umbral)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Primer detección de arrastre real, recién reiniciar simulación
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Solo dejar que la simulación se detenga gradualmente si realmente se arrastró
        if (d._isDragging) {
  ...
  simulation.on('tick', () => {
    ...
    // Actualizar posición de etiquetas de borde (sin rotación, visualización horizontal más clara)
    ...
    // Actualizar fondo de etiquetas de borde
  ...
  // Clic en área en blanco para cerrar panel de detalles
  svg.on('click', () => {
    selectedItem.value = null
    node.attr('stroke', '#fff').attr('stroke-width', 2.5)
    linkGroup.selectAll('path').attr('stroke', '#C0C0C0').attr('stroke-width', 1.5)
...
// Escuchar interruptor de visualización de etiquetas de borde
...
/* Estilos del indicador después de que termina la simulación */
